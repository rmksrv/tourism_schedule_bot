...
def start(update: Update, context: CallbackContext):
    info_message = f"Привет, я бот {BOT_NAME}. Я создан, чтобы потешить эго моего создателя. А еще я могу " \
...
def action_menu(update: Update, context: CallbackContext):
    reply_markup = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text=BotCommands.Today.value), KeyboardButton(text=BotCommands.Tomorrow.value)],
...
def today_schedule(update: Update, context: CallbackContext):
    ...
    context.bot.send_message(update.effective_chat.id, info_message, parse_mode=ParseMode.HTML)
